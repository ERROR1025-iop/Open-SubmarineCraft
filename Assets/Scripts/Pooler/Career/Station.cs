    ...
    public class Station : MonoBehaviour
    {
        ...
        public bool isCanAddCargos(int[] cargos)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    l_soildCount += cargos[i];
                }
                else if (block.isCanStoreInWarehouse() == 2)
                {
                    l_liquidCount += cargos[i];
                }

                if (l_soildCount > canStoreSoild || l_liquidCount > canStoreLiquid)
                {
                    return false;
                }
            }
        ...
        {
            if (Pooler.mainStationRemaindeCargoCounts == null && !GameSetting.isAndroid)
            {
