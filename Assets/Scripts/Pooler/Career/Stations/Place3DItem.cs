...
namespace Scraft
{
    ...
    public class Place3DItem : MonoBehaviour
    {
        ...
        EarthCurvatureObjectController earthCurvatureObjectController;

        public delegate void PlaceFinishDelegate(Station station);
        public event PlaceFinishDelegate OnPlaceFinish;

        void Start()
        {
            Camera3DWorld = Camera.main;            
            boxColliders = GetComponents<BoxCollider>();
            earthCurvatureObjectController = GetComponent<EarthCurvatureObjectController>();
            earthCurvatureObjectController.enabled = false;
        ...
        public void setMaterialBlocksCoor(IPoint[] coors)
        {
            materialBlocksCoor = coors;
        }
        ...
        void onTrueButtonClick()
        {
            if(transform.position.y > 0)
            {
                ...
                isPlace = true;
                earthCurvatureObjectController.enabled = true;
                earthCurvatureObjectController.ResetOriginalY();
                Destroy(this);
                ...
                //消耗耗材
                if(materialBlocksCoor != null)
                {
                    BlocksEngine blocksEngine = BlocksEngine.instance;
                    foreach (IPoint coor in materialBlocksCoor)
                    {
