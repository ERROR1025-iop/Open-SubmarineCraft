...
{    
    public class StationUpgrade : MonoBehaviour
    {
        ...
        public void initialized(StationInfo stationInfo)
        {
            ...
            del10Button.gameObject.SetActive(false);
            del100Button.gameObject.SetActive(false);
            del1000Button.gameObject.SetActive(false);
        }
        ...
        void delBlock(int count)
        {
            ...
            {
                station.removeCargos(selectBlockId, count);
                initializedStoreCargoCount();
                initializedStoreCargoCell(station.cargoCounts);
            }
        }
        ...
        void onBuildButtonClick()
        {
            ...
            if(cell != null)
            {
                if (station.IsContainCargo(cell.componentInfo.synt) || !GameSetting.isCareer || Application.isEditor)
                {
                    GameObject go = Instantiate(Resources.Load("Stations/Prefabs/" + cell.componentInfo.name)) as GameObject;
                    go.transform.SetParent(GameObject.Find("runtime_gen").transform, true);
                    StationComponent stationComponent = go.GetComponent<StationComponent>();
