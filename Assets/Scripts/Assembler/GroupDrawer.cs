...
namespace Scraft
{
    public class GroupDrawer
    {
        ...
        public GroupDrawer(GroupDpartsSelector groupDpartsSelector, string groupName, IScrollView cardIScrollView, FileInfo folder, int rank)
        {
        ...
        private void loadThumbnail()
        {
            ...
            try
            {
                texture2D = IUtils.loadTexture2DFromSD(GamePath.groupsThumbnailFolder + groupName + "/" + getDrawerName() + ".thu");
                isLoadThu = true;
            }
            catch
            {
        ...
        public void createThumbnailImage()
        {
            ...
            Texture2D texture2D = AssemblerUtils.createDpartThumbnailImage(Assembler.dpartsEngine, folder.FullName, GamePath.groupsThumbnailFolder + groupName + "/", getDrawerName());
            thumbnailImage.sprite = Sprite.Create(texture2D, new Rect(0, 0, texture2D.width, texture2D.height), Vector2.zero);
            IUtils.resetImageSize(thumbnailImage, new Vector2(60, 40));
        }
        ...
        void onDrawerClick()
        {
            ...
            groupDpartsSelector.onDrawerClick(groupName, getDrawerName(), rank);
        }
