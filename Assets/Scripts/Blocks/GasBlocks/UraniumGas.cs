...
{
    public class UraniumGas : GasBlock
    {
        float probability;
        bool isInspire;
        int collideDir;

        public UraniumGas(int id, GameObject parentObject, GameObject blockObject)
            : base(id, parentObject, blockObject)
        {
            initBlock("uraniumGas", "null");
        ...
        {
            UraniumGas block = new UraniumGas(blockId, parentObject, blockObject);
            block.initGasBlock(blocksManager, 3745, "uraniumLiquid");
            return block;
        }
        ...
        {
            float dt = Uranium.selftAt + temperature * 1000;
            if (isInspire)
            {
                dt = Uranium.inspireAt + temperature * 2000;
            }
            addHeatQuantity(dt);
        }
