...
// Vertex shader
    ...
    //  Calculate new ClipPos (optimized)
        ...
        // ####################################################################
        // 新增：同步山体的“基于玩家距离的下沉逻辑”
        // ####################################################################
        // 1. 计算水面顶点到主角的XZ平面距离（与山体Shader逻辑完全一致）
        float dx = worldPos.x - _PlayerPos.x;
        float dz = worldPos.z - _PlayerPos.z;
        float distToPlayer = length(float2(dx, dz));

        // 2. 计算下沉量：主角范围内不下沉，超出后按曲线下沉
        float sinkAmount = 0.0;
        if (distToPlayer > _NoDropRange)
        {
            // 有效距离 = 实际距离 - 不下沉范围（从范围边界开始下沉）
            float effectiveDist = distToPlayer - _NoDropRange;
            effectiveDist = max(effectiveDist, 0.0); // 避免负数导致异常
            
            // 按幂函数计算下沉系数（与山体Shader一致，确保曲线同步）
            float sinkT = pow(effectiveDist / _SinkRadius, _SinkPower);
            sinkAmount = sinkT * _SinkMaxDrop; // 最终下沉量（Y轴偏移）
        }

        // 3. 应用下沉效果（修改世界空间Y轴，负数=下沉）
        worldPos.y -= sinkAmount;

        // 4. 更新裁剪空间位置（将修改后的世界位置转换为屏幕位置）
        o.pos = mul(UNITY_MATRIX_VP, worldPos);
        // ####################################################################
        // 新增结束
        // ####################################################################
...
//  Fragment shader
    ...
    //  This does not work on metal if we are using deferred rendering and enable ZWrite... - > force DepthNormalTexture?
        ...
        float sceneDepth = GetSceneDepth(0);
...
worldNormal = normalize(worldNormal);
    ...
    //    half3 lightColor = _LightColor0.rgb * atten;
            ...
            #if defined (SPOT)
                float4 lightCoordSpot = float4(lightCoord, 1);
                atten *= tex2D(_LightTextureB0, lightCoordSpot.xy / lightCoordSpot.w + 0.5).r;
            #endif
        #endif
        half3 lightColor = _LightColor0.rgb * atten;
