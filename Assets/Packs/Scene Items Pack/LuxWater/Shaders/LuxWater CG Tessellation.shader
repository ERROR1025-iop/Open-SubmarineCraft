...
//		_PixelSnap						("    Enable Pixel Snapping", Float) = 0
		...
		[Header(Mountain Sync Sink)] // 新增：山体同步下沉参数组
		_PlayerPos ("Player World Position", Vector) = (0,0,0,1) // 主角位置（与山体Shader一致）
		_NoDropRange ("No Sink Range (Player Distance)", Float) = 300.0 // 主角周围不下沉范围
		_SinkRadius ("Sink Effect Radius", Float) = 18.0 // 下沉效果总范围
		_SinkMaxDrop ("Sink Max Y Offset", Float) = 2.0 // 最大下沉量
		_SinkPower ("Sink Curve Power", Range(0.1,5)) = 1.0 // 下沉曲线陡峭度
	}
